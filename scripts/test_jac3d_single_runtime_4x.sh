...
XSTREAMS=(1 2 4 8)
TASK_SIZES_STR="${TASK_SIZES_4X:-384 320 256 192}"
...
if [ "${#TASK_SIZES[@]}" -ne 4 ]; then
    echo "Expected exactly 4 task sizes, got ${#TASK_SIZES[@]} (${TASK_SIZES[*]})." >&2
    exit 1
fi

mkdir -p results_scheduler_compare
RESULTS="results_scheduler_compare/benchmark_single_runtime_4x.txt"
SUMMARY="results_scheduler_compare/summary_single_runtime_4x.csv"
echo "scheduler,xstreams,size1,chunks1,size2,chunks2,size3,chunks3,size4,chunks4,task_count,total_time_seconds,total_real_time_nanos,total_mono_time_nanos,total_steal_operations,total_stolen_tasks,verification" > "$SUMMARY"
echo "Single-runtime 4-task Jacobi benchmark" > "$RESULTS"
...
for scheduler in old new; do
    ...
    for xstreams in "${XSTREAMS[@]}"; do
        output_file="results_scheduler_compare/jac3d_${scheduler}_single_runtime_4x_x${xstreams}.txt"
        ...
        echo "$scheduler,$xstreams,${TASK_SIZES[0]},${TASK_CHUNKS[0]},${TASK_SIZES[1]},${TASK_CHUNKS[1]},${TASK_SIZES[2]},${TASK_CHUNKS[2]},${TASK_SIZES[3]},${TASK_CHUNKS[3]},4,$total_time,$total_nanos,$total_mono,$total_steal_ops,$total_stolen_tasks,$verification" >> "$SUMMARY"
        echo "$scheduler x=$xstreams {${TASK_SIZES[0]},${TASK_CHUNKS[0]}} {${TASK_SIZES[1]},${TASK_CHUNKS[1]}} {${TASK_SIZES[2]},${TASK_CHUNKS[2]}} {${TASK_SIZES[3]},${TASK_CHUNKS[3]}} total_time=$total_time total_steal_ops=$total_steal_ops total_stolen_tasks=$total_stolen_tasks verification=$verification" >> "$RESULTS"
    done
done
