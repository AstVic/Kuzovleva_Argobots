...
for scheduler in old new; do
    ...
    for xstreams in "${XSTREAMS[@]}"; do
        for threads in "${THREADS[@]}"; do
            output_file="results_scheduler_compare/jac3d_${scheduler}_single_runtime_4x_x${xstreams}_t${threads}.txt"
            ...
            echo "$scheduler,$xstreams,$threads,4,$total_time,$total_nanos,$total_steal_ops,$total_stolen_tasks,$verification" >> "$SUMMARY"
            echo "$scheduler x=$xstreams t=$threads tasks=4 sizes=${TASK_SIZES[*]} total_time=$total_time total_steal_ops=$total_steal_ops total_stolen_tasks=$total_stolen_tasks verification=$verification" >> "$RESULTS"
        done
    done
done
