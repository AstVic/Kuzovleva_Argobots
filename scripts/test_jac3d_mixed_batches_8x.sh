...
for scheduler in old new; do
    ...
    for threads in "${THREADS[@]}"; do
        for batch in $(seq 1 "$MIXED_TASK_BATCHES"); do
            ...
            echo "$scheduler x=$XSTREAMS t=$threads batch=$batch parallel_tasks=8 sizes=${MIXED_TASK_SIZES[*]} total_time=$total_time total_steal_ops=$total_steal_ops total_stolen_tasks=$total_stolen_tasks verification=$verification" >> "$RESULTS"
        done
    done
done
